...
func renderTemplate(req modules.Request) ([]byte, string, error) {
	if req.Step.Args == nil {
		return nil, "", fmt.Errorf("template.render requires args.src and args.dest")
	}

	src, ok := readString(req, "src")
	if !ok || src == "" {
		return nil, "", fmt.Errorf("template.render requires args.src")
	}
	dest, ok := readString(req, "dest")
	if !ok || dest == "" {
		return nil, "", fmt.Errorf("template.render requires args.dest")
	}
