...
func TestLoadMapping(t *testing.T) {
	...
	if got := wf.Steps[0].With["cmd"]; got != "echo hello" {
		t.Fatalf("step0 cmd mismatch: %v", got)
	}
	if got := wf.Steps[0].With["dir"]; got != "/tmp" {
		t.Fatalf("step0 dir mismatch: %v", got)
	}

	env, ok := wf.Steps[2].With["env"].(map[string]any)
	if !ok {
	...
	if got := wf.Steps[3].With["name"]; got != "nginx" {
		t.Fatalf("step3 name mismatch: %v", got)
	}
	if got := wf.Steps[4].With["src"]; got != "nginx.conf.j2" {
		t.Fatalf("step4 src mismatch: %v", got)
	}
	if got := wf.Steps[4].With["dest"]; got != "/etc/nginx/nginx.conf" {
		t.Fatalf("step4 dest mismatch: %v", got)
	}
	if got := wf.Steps[5].With["state"]; got != "started" {
